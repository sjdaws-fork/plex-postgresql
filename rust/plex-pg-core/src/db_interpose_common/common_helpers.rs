...
pub fn rust_get_type_name(tinfo: *mut c_void) -> *const c_char {
    if tinfo.is_null() {
...
pub fn rust_rewrite_blobs_schema_migrations(
    _sql: *const c_char,
...
pub fn rust_simple_str_replace(
    ...
    unsafe {
        ...
        let old_len = CStr::from_ptr(old_ptr).to_bytes().len();
        let new_len = CStr::from_ptr(new_ptr).to_bytes().len();
