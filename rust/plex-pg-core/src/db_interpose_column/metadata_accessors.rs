...
pub(super) fn column_count_impl(p_stmt: *mut sqlite3_stmt) -> c_int {
    ...
    get_orig_sqlite3_column_count().map(|f| unsafe { f(p_stmt) }).unwrap_or(0)
}
...
pub(super) fn column_name_impl(p_stmt: *mut sqlite3_stmt, idx: c_int) -> *const c_char {
    ...
    if !raw_pg_stmt.is_null() && unsafe { (&*raw_pg_stmt).is_pg != 0 } {
        ...
        // to avoid ABBA deadlock between stmt mutex and LOGGER mutex.
        {
            ...
            if !pg_stmt.col_names.is_null()
                && idx >= 0
                && idx < pg_stmt.num_col_names
            {
                ...
                && idx < pg_stmt.num_cols
            {
                ...
                use_orig = pg_stmt.result.is_null()
                    && pg_stmt.col_names.is_null();
            }
        }
        // Mutex released here.
    ...
    if use_orig {
        result = get_orig_sqlite3_column_name().map(|f| unsafe { f(p_stmt, idx) }).unwrap_or(ptr::null());
    }
    result
}
...
pub(super) fn data_count_impl(p_stmt: *mut sqlite3_stmt) -> c_int {
    ...
    get_orig_sqlite3_data_count().map(|f| unsafe { f(p_stmt) }).unwrap_or(0)
}
